...
function getStockStatus(warehouse:number[]):string{
...
let discount=0;
if(category=="Books") discount=0.1;
if(category=="Electronics") discount=0.15;
...
products.forEach(p=>{console.log(p.name+" | "+p.category+" | "+getStockStatus(p.warehouse)+" | "+"rating "+getAverageRating(p.ratings)+" | "+"price $"+getDiscount(p.price,p.category));
